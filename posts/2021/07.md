...
- `export default foo` と `export { foo as default }`は動作が異なるので注意
- `export default foo` の場合、 `foo` は expression (式)として export される
- `export { foo as default }` の場合、 `foo` は reference (参照)として export される
- (To be continued...)
